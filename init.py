...
def get_settings() -> dict:
    """Parse settings.txt into a dict; creates the file with defaults if it doesn't exist, and backfills any missing settings (except debug, which is always opt-in)."""
    ...
    if file.is_file():
        ...
        missing = {k: v for k, v in SETTING_DEFAULTS.items() if k not in rules}
        if missing:
            additions = []
            for key, val in missing.items():
                rules[key] = val
                additions.append(f"{key}={'TRUE' if val is True else 'FALSE' if val is False else val}")
...
def run() -> None:
    """Run first-time and ongoing setup: welcome message, dependency checks, GitHub auth, and Gemini API verification."""
